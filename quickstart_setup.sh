...
#echo Copying enron files from `pwd`/maildir/ to ${HDFS_ENRON_INDIR}/maildir/. This may take a few minutes.
#hadoop fs -mkdir -p ${HDFS_ENRON_INDIR}/maildir/ || die "Unable to create ${HDFS_ENRON_INDIR}"
#hdfs dfs -copyFromLocal -f "./" "${HDFS_ENRON_INDIR}/maildir" || die "Unable to copy enron files from local to HDFS"
